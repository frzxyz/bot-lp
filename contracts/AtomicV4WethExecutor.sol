...
/// @notice Owner-only atomic WETH/Uniswap-V4 lifecycle wrapper. Swap calldata must be
...
contract AtomicV4WethExecutor {
    ...
    address public immutable WETH; address public immutable POSITION_MANAGER;
    ...
    event Opened(uint256 indexed tokenId,address indexed token,bytes32 indexed poolId,uint256 wethIn,uint256 tokenOut);
    event InventoryOpened(uint256 indexed tokenId,address indexed token,bytes32 indexed poolId,uint256 wethCap,uint256 tokenCap);
    ...
    constructor(address owner_,address weth,address posm,address permit2,address swapTarget){
        if(owner_==address(0)||weth==address(0)||posm==address(0)||permit2==address(0)||swapTarget==address(0))revert Invalid();
        owner=owner_;WETH=weth;POSITION_MANAGER=posm;PERMIT2=permit2;SWAP_TARGET=swapTarget;
    }
    ...
    struct Open {address token;PoolIdentity pool;uint256 wethAmount;uint256 swapAmount;uint256 minTokenOut;uint256 amount0Min;uint256 amount1Min;uint256 deadline;address swapTarget;bytes swapData;bytes mintData;}
    function atomicOpen(Open calldata p) external onlyOwner live returns(uint256 tokenId){
        ...
        if(p.wethAmount==0||p.swapAmount==0||p.swapAmount>=p.wethAmount||p.minTokenOut==0||p.amount0Min==0||p.amount1Min==0)revert Invalid();
        _target(p.swapTarget,p.swapData);_modify(p.mintData);_clean(p.token);_pull(WETH,msg.sender,p.wethAmount);
        _approve(WETH,SWAP_TARGET,p.swapAmount);uint256 tb=_bal(p.token);_raw(SWAP_TARGET,p.swapData);_approve(WETH,SWAP_TARGET,0);
        uint256 tokenOut=_bal(p.token)-tb;if(tokenOut<p.minTokenOut)revert Slippage();
        _permit(WETH);_permit(p.token);uint256 next=IV4PositionManager(POSITION_MANAGER).nextTokenId();_raw(POSITION_MANAGER,p.mintData);
        ...
        _revoke(WETH);_revoke(p.token);_sweep(WETH);_sweep(p.token);emit Opened(tokenId,p.token,p.pool.poolId,p.wethAmount,tokenOut);
    }
    /// @notice No-swap open funded by exact owner inventory caps; unused dust is refunded.
    struct InventoryOpen {address token;PoolIdentity pool;uint256 wethAmount;uint256 tokenAmount;uint256 amount0Min;uint256 amount1Min;uint256 deadline;bytes mintData;}
    function atomicInventoryOpen(InventoryOpen calldata p) external onlyOwner live returns(uint256 tokenId){
        ...
        if(p.wethAmount==0||p.tokenAmount==0||p.amount0Min==0||p.amount1Min==0)revert Invalid();
        _clean(p.token);_pull(WETH,msg.sender,p.wethAmount);_pull(p.token,msg.sender,p.tokenAmount);
        _permit(WETH);_permit(p.token);uint256 next=IV4PositionManager(POSITION_MANAGER).nextTokenId();_raw(POSITION_MANAGER,p.mintData);
        tokenId=next;if(IV4PositionManager(POSITION_MANAGER).ownerOf(tokenId)!=owner)revert PoolMismatch();_checkPosition(tokenId,p.pool);
        _revoke(WETH);_revoke(p.token);_sweep(WETH);_sweep(p.token);emit InventoryOpened(tokenId,p.token,p.pool.poolId,p.wethAmount,p.tokenAmount);
    }
    struct Close {uint256 tokenId;address token;PoolIdentity pool;uint128 liquidity;uint256 amount0Min;uint256 amount1Min;uint256 minSwapOut;uint256 minTotalWeth;uint256 deadline;address swapTarget;bytes closeData;bytes swapData;}
    function atomicClose(Close calldata p) external onlyOwner live returns(uint256 settlementAmount,uint256 tokenAmount){
        ...
        _raw(POSITION_MANAGER,p.closeData);settlementAmount=_bal(WETH);tokenAmount=_bal(p.token);
        _sweep(WETH);_sweep(p.token);emit Closed(p.tokenId,p.token,settlementAmount,tokenAmount);
    }
    ...
    function _clean(address t) internal view {if(_bal(WETH)!=0||_bal(t)!=0)revert DirtyBalance();} function _bal(address t) internal view returns(uint256){return IV4ERC20(t).balanceOf(address(this));}
