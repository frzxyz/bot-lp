...
import { discoverV4Erc20Pools } from "./chain/v4/discover.js";
...
export const EXECUTOR=process.env.ATOMIC_V4_WETH_EXECUTOR_ADDRESS||ethers.ZeroAddress;
...
async function eligiblePool(token:string,requestedPoolId?:string){
  const t=ethers.getAddress(token);if(t===ethers.getAddress(WETH))throw Error("candidate token cannot be WETH");
  if(!requestedPoolId)throw Error("candidate poolId is required for WETH settlement");
  const found=selectEligiblePool(await discoverV4Erc20Pools(t,WETH),requestedPoolId);
  if(!found)throw Error("no discovered live WETH-paired eligible V4 pool");
  return found;
}
...
export async function generateOpenPlan(token:string,budget:bigint,outFile:string,slippageBps=500,widthSpacings=8,requestedPoolId?:string){
  if(budget<=0n)throw Error("budget must be positive raw WETH");
  ...
  const k=await kyber(WETH,candidate,swapAmount,slippageBps), retained=budget-swapAmount;
  const pool=sdkPool(p,m0,m1), weth0=p.poolKey.currency0.toLowerCase()===WETH.toLowerCase(), avail0=weth0?retained:k.out,avail1=weth0?k.out:retained;
  ...
  const plan={token:candidate,pool:{...p.poolKey,poolId:p.poolId},wethAmount:String(budget),swapAmount:String(swapAmount),minTokenOut:String(floor(k.out,slippageBps)),amount0Min:String(amount0Min),amount1Min:String(amount1Min),deadline,swapTarget:env.kyberRouter,swapData:k.data,mintData:call.calldata,meta:{chainId:4663,executor:EXECUTOR,payer:EXECUTOR,nftRecipient:OWNER,walletInventoryUsed:false,selection:"discovered-weth-highest-liquidity",currentTick:p.tick,tickLower,tickUpper,sqrtPriceX96:String(p.sqrtPriceX96),quotedTokenOut:String(k.quoted),builtTokenOut:String(k.out),generatedAt:new Date().toISOString()}};
...
export async function generateInventoryOpenPlan(outFile:string,slippageBps=500,widthSpacings=8){
  if(slippageBps<1||slippageBps>2000)throw Error("slippage bps must be 1..2000");
  const p=await eligiblePool(PENGUZILLA,PENGU_POOL_ID),[m0,m1,wethWallet,tokenWallet]=await Promise.all([tokenMeta(p.poolKey.currency0),tokenMeta(p.poolKey.currency1),new ethers.Contract(WETH,ERC20_ABI,provider).balanceOf(OWNER) as Promise<bigint>,new ethers.Contract(PENGUZILLA,ERC20_ABI,provider).balanceOf(OWNER) as Promise<bigint>]);
  const reserve=2_000_000n,maxEntry=25_000_000n,spendable=wethWallet>reserve?wethWallet-reserve:0n,wethCap=spendable<maxEntry?spendable:maxEntry;
  // Deliberately retain 1% PENG inventory: the plan cannot sweep the wallet even when PENG is limiting.
  const tokenCap=tokenWallet*99n/100n;strict(wethCap,"spendable WETH after 2 WETH reserve");strict(tokenCap,"PENG inventory cap");
  ...
  if(!(tickLower<p.tick&&p.tick<tickUpper))throw Error("generated range is not strictly in range");
  const pool=sdkPool(p,m0,m1),weth0=p.poolKey.currency0.toLowerCase()===WETH.toLowerCase(),avail0=weth0?wethCap:tokenCap,avail1=weth0?tokenCap:wethCap;
  ...
  const wethPull=weth0?max0:max1,tokenPull=weth0?max1:max0;
  ...
  const amount0Min=strict(floor(BigInt(position.amount0.quotient.toString()),slippageBps),"amount0Min"),amount1Min=strict(floor(BigInt(position.amount1.quotient.toString()),slippageBps),"amount1Min");
  const plan={token:PENGUZILLA,pool:{...p.poolKey,poolId:p.poolId},wethAmount:String(wethPull),tokenAmount:String(tokenPull),amount0Min:String(amount0Min),amount1Min:String(amount1Min),deadline,mintData:call.calldata,meta:{chainId:4663,executor:EXECUTOR,payer:EXECUTOR,nftRecipient:OWNER,walletInventoryUsed:true,noSwap:true,wethWallet:String(wethWallet),tokenWallet:String(tokenWallet),wethReserve:String(reserve),maxEntryUsdg:String(maxEntry),currentTick:p.tick,tickLower,tickUpper,balancedAmount0:String(position.amount0.quotient),balancedAmount1:String(position.amount1.quotient),generatedAt:new Date().toISOString()}};
  fs.mkdirSync(path.dirname(outFile),{recursive:true});fs.writeFileSync(outFile,JSON.stringify(plan,null,2)+"\n");return plan;
}
export async function generateClosePlan(tokenId:string,outFile:string,slippageBps=500){
  ...
  if(id.toLowerCase()!==PENGU_POOL_ID)throw Error("tokenId is not the exact PENGUZILLA pool");
  ...
  const a0=BigInt(pos.amount0.quotient.toString()),a1=BigInt(pos.amount1.quotient.toString()), weth0=p.currency0.toLowerCase()===WETH.toLowerCase(), tokenExpected=weth0?a1:a0, wethExpected=weth0?a0:a1;
  strict(tokenExpected,"expected token principal");strict(wethExpected,"expected WETH principal");
  ...
  // Close is stage one only. The executor ignores the legacy swap fields and sweeps both
  // exact receipt deltas to OWNER; runtime durably records tokenAmount before liquidation.
  const plan={tokenId:String(tokenId),token:PENGUZILLA,pool:{...p,poolId:id},liquidity:String(liquidity),amount0Min:String(min0),amount1Min:String(min1),minSwapOut:"0",minTotalWeth:"0",deadline,swapTarget:ethers.ZeroAddress,closeData,swapData:"0x",meta:{chainId:4663,executor:EXECUTOR,positionOwner:OWNER,closeRecipient:EXECUTOR,walletInventoryUsed:false,twoStageSettlement:true,currentTick:Number(s0.tick),tickLower:lo,tickUpper:hi,expectedTokenIn:String(tokenExpected),expectedWethFromLp:String(wethExpected),generatedAt:new Date().toISOString()}};
  fs.mkdirSync(path.dirname(outFile),{recursive:true});fs.writeFileSync(outFile,JSON.stringify(plan,null,2)+"\n");return plan;
}
